...
function formatDeliveryLinks_(deliveryValue) {
  try {
    if (Array.isArray(deliveryValue)) return deliveryValue;
    const s = String(deliveryValue || '').trim();
    if (s.startsWith('[') && s.endsWith(']')) return JSON.parse(s);
  } catch (e) {}
  const one = String(deliveryValue || '').trim();
  return one ? [one] : [];
}
...
function createCheckout_(payload) {
  ...
  const skusRaw = payload.skus || payload.Skus || '';
  const skus = skusRaw ? String(skusRaw).split(',').map(function(s){return String(s).trim();}).filter(Boolean) : [];

  const email = payload.email || payload.customer_email || '';
  const catalog = getPublishedCatalog_();
  const catalogItems = (catalog && catalog.items) ? catalog.items : [];
  const item = sku ? catalogItems.find(function (x) { return x.sku === sku; }) : null;
  const multiItems = skus.length ? skus.map(function(s){ return catalogItems.find(function(x){ return x.sku === s; }); }).filter(Boolean) : [];


  if (!item && !multiItems.length) return { ok: false, error: 'SKU not found' };
  ...
  const totalCents = multiItems.length ? multiItems.reduce(function(sum,it){ return sum + Number(it.price_cents || 0); }, 0) : Number((item && item.price_cents) || 0);
  const amount = (Number(totalCents || 0) / 100).toFixed(2);
  ...
  const pf = {
    ...
    amount: amount,
    item_name: multiItems.length ? ('StudyHub Bundles (' + multiItems.length + ' items)') : ((item && item.title) || sku),
    item_description: multiItems.length ? ('StudyHub bundle checkout: ' + skus.join(', ')) : (((item && item.description) || ('StudyHub purchase ' + sku))),
    email_address: email,
  ...
  upsertOrder_(orderId, multiItems.length ? skus.join('|') : sku, email, totalCents, 'PENDING', JSON.stringify(payload));
...
function sendDelivery_(orderId) {
  ...
  const body = [
    ...
    'SKU: ' + (order.sku || ''),
    'Delivery link(s):',
    ...formatDeliveryLinks_(order.delivery_url || '').map(function(u){ return ' - ' + u; }),
