...
function renderDeliveryLinks(delivery){
  const linkEl = document.getElementById('deliveryLink');
  try{
    let links = delivery;
    if (typeof links === 'string'){
      const t = links.trim();
      if ((t.startsWith('[') && t.endsWith(']'))){ links = JSON.parse(t); }
    }
    if (Array.isArray(links)) {
      const p = linkEl ? linkEl.closest('p') : null;
      if (!p) return;
      p.innerHTML = '<strong>Downloads:</strong>';
      ...
      links.forEach((u, idx) => {
        ...
        a.href = String(u); a.target='_blank'; a.rel='noopener';
        a.textContent = 'Download link ' + (idx+1);
  ...
  if (delivery && linkEl) linkEl.href = String(delivery);
}
  ...
  while (attempts < maxAttempts) {
    ...
    try {
      ...
      if (status === 'COMPLETE') {
        ...
        if (deliveryUrl) renderDeliveryLinks(deliveryUrl);
