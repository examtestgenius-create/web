...
function render(item){
  ...
  const price = moneyZarFromCents(item.price_cents);
  ...
  return `
    <div class="detail-layout">
      <section class="detail-panel card-surface">
        ...
        <h2>${sku}</h2>
        <p class="product-note">${desc[0] || 'Exam paper bundle with memos included.'}</p>
        <div class="badge-row">
...
async function loadPackageDetail(){
  ...
  try {
    ...
    if (!item){
      detailStatus.innerHTML = `<span class="eyebrow">Package detail</span><h2>Bundle not found</h2><p>No bundle with SKU <code>${sku}</code>.</p>`;
      return;
    }

    detailStatus.innerHTML = `<span class="eyebrow">Package detail</span><h2>${sku}</h2><p class="product-note">Ready for PayFast checkout.</p>`;
    detailRoot.innerHTML = render(item);

  } catch(err){
    console.error(err);
    detailStatus.innerHTML = '<span class="eyebrow">Package detail</span><h2>Catalog unavailable</h2><p class="product-note">Please try again later.</p>';
  }
}
