...
async function fetchStudyHubCatalog() {
  ...
  for (const url of tryUrls) {
    try {
      ...
      errors.push(`${url}: ${err.name === 'AbortError' ? 'Timeout' : (err.message || err)}`);
    }
  }
  throw new Error(errors.join('\n'));
}
function inferGradeFromSku(sku) { const m = String(sku || '').match(/SH-G(\d{1,2})-/i); return m && m[1] ? m[1] : ''; }
function normalizeCatalogItem(item) {
  ...
  if ((!fromYear || !toYear) && /^\d{4}-\d{4}$/.test(String(yearRange))) { const parts = String(yearRange).split('-'); fromYear = fromYear || parts[0]; toYear = toYear || parts[1]; }
