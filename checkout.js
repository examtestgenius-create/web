...
const cartSkus = (window.StudyHubCart && StudyHubCart.load) ? StudyHubCart.load() : [];
...
async function loadCheckout() {
  ...
  skuField.value = sku || (cartMode ? cartSkus.join(', ') : '');
  try {
    const { payload } = await fetchStudyHubCatalog();
    const items = payload.items || payload.packages || [];
    if (cartMode) {
      titleEl.textContent = 'Checkout — Basket';
      introEl.textContent = 'You are checking out multiple bundles in one payment.';
      const selected = cartSkus.map(s => items.find(v => String(v.SKU || v.sku || '') === s)).filter(Boolean);
      const totalCents = selected.reduce((sum, it) => sum + Number(it.Price_Cents || it.price_cents || 0), 0);
      currentItem = { Price_Cents: totalCents, Subject_Name: 'MULTI', Province_Filter: 'ALL', Coverage_From_Year: 2022, Coverage_To_Year: 'Onward', Included_File_Count: selected.reduce((sum, it) => sum + Number(it.Included_File_Count || it.included_file_count || 0), 0) };
      priceEl.textContent = new Intl.NumberFormat('en-ZA', { style: 'currency', currency: 'ZAR' }).format(totalCents/100);
      badgesEl.innerHTML = `<span class="badge">${selected.length} bundles</span><span class="badge">One payment</span>`;
      fromYearMeta.textContent = 2022;
      toYearMeta.textContent = 'Onward';
