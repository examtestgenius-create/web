...
const CFG = {
  site: {
    ...
    whatsappE164: '+27XXXXXXXXX' // TODO: set your official WhatsApp number
  },
  payfast: {
  ...
  data: {
    catalogUrl: 'assets/data/catalog.json'
  }
};
...
async function initCatalog() {
  ...
  const res = await fetch(CFG.data.catalogUrl);
  ...
  function matches(x) {
    return (!state.grade || x.grade === state.grade)
      && (!state.subject || x.subject === state.subject)
      && (!state.term || x.term === state.term)
...
async function initProduct() {
  ...
  // Set PayFast process URL based on env
