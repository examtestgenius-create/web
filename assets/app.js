...
function getCart() {
  try {
    return JSON.parse(localStorage.getItem("studyhub_cart") || "[]");
  } catch (e) {
    return [];
  }
}
...
// Expose for home page inline script
...
function renderCart() {
  ...
  if (cart.length === 0) {
    list.innerHTML =
      '<div class="card"><b>Your cart is empty.</b><div class="muted" style="margin-top:6px">Browse packs to add items.</div><div style="margin-top:12px"><a class="btn btn-primary" href="catalog.html">Browse Packs</a></div></div>';
